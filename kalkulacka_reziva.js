...
const DEFAULT_MATERIALS = [
  ...
  { id: 2, name: 'KVH hranoly', price: 16500, density: 450, emailCode: 'KVH Si', sizes: [
...
function fmtM3(n)  { return fmt(n, 3) + '\u00a0m³'; }
function fmtKc(n)  { return fmt(Math.round(n)) + '\u00a0Kč'; }
function fmtKg(n)  { return fmt(Math.round(n)) + '\u00a0kg'; }
  ...
  if (isLastF && isLastR) {
    ...
    if (nxt) {
      document.querySelector(
        `tr[data-id="${nxt.id}"] input[data-field="w"]`
      )?.focus();
    }
  } else {
    const nextField = FIELDS[FIELDS.indexOf(field) + 1];
    document.querySelector(
      `tr[data-id="${rowId}"] input[data-field="${nextField}"]`
    )?.focus();
  }
}
...
function addMaterial() {
  ...
  if (!name) { alert('Zadejte název materiálu.'); return; }
...
function addSize() {
  ...
  if (!w || !h) { alert('Zadejte šířku a výšku průřezu.'); return; }

  const lengths = lengthsStr.split(',')
    .map(v => parseFloat(v.trim()))
    .filter(v => !isNaN(v) && v > 0);
  if (!lengths.length) { alert('Zadejte alespoň jednu dostupnou délku.'); return; }

  if (!mat.sizes) mat.sizes = [];
  if (mat.sizes.find(s => s.w === w && s.h === h)) {
    alert(`Průřez ${w} × ${h} cm již v seznamu existuje.`); return;
  }
...
function exportPDF() {
  ...
  document.getElementById('printDate').textContent     = new Date().toLocaleDateString('cs-CZ');
...
function buildEmailText() {
  ...
  if (inclWght) footer += `Celková hmotnost: ${document.getElementById('sumWeight').textContent}\n`;
  ...
  if (link.length <= 1900) {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(() => {
        showToast('Text nabídky byl zkopírován do schránky. Vložte ho do emailu ručně.');
      }).catch(() => {
        showToast('Nepodařilo se zkopírovat. Text je příliš dlouhý pro email — zkuste tisk.');
      });
    } else {
      showToast('Text nabídky je příliš dlouhý pro mailto. Použijte Tisk / PDF.');
    }
  }
}
...
function buildQuoteEmailText() {
  ...
  const total = rows.reduce((s, r) => {
    const m3 = ((parseFloat(r.w) || 0) / 100) * ((parseFloat(r.h) || 0) / 100)
               * (parseFloat(r.l) || 0) * (parseFloat(r.n) || 0);
...
function sendQuoteEmail() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(() => {
      showToast('Cenová nabídka zkopírována do schránky. Vložte ji do emailu.');
    }).catch(() => {
